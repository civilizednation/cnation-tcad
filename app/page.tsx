...
function ContourSvg({ implants, idPrefix, label }: { implants: ActiveImplant[]; idPrefix: string; label: string }) {
  ...
  return (
    <svg viewBox="0 0 640 330" role="img" aria-label={`${label} 조건의 buried gate cell transistor implant 농도 분포`}>
      ...
      {implants.map((implant,index) => { const model=SOURCE_MODEL[implant.source]??SOURCE_MODEL.Boron; const cy=52+(model.rp(implant.energy)/.58)*234; const doseRatio=implant.dose/2.6e13; const opacity=clamp(0.42+0.30*doseRatio,0.42,1); const sizeScale=clamp(0.82+0.22*doseRatio,0.8,1.35); const rx=184*sizeScale, ry=72*sizeScale; const coreScale=clamp(0.34+0.24*doseRatio,0.3,0.62); return <g key={`${implant.source}-${implant.energy}-${index}`}><ellipse cx="320" cy={cy} rx={rx} ry={ry} fill={`url(#${doseId})`} opacity={opacity} filter={`url(#${glowId})`}/><ellipse cx="320" cy={cy} rx={rx*coreScale} ry={ry*coreScale} fill="#ef3f23" opacity={clamp(0.35+0.35*doseRatio,0.35,0.85)}/><line x1="468" x2="566" y1={cy} y2={cy} stroke={index===0?"#e04f2c":"#008fb0"} strokeWidth="2" strokeDasharray="5 5"/><text x="570" y={cy+4} fontSize="12" fill="#28424f" textAnchor="end">{`${implant.energy} keV peak`}</text></g>; })}
