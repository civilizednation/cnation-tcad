...
function ContourSvg({ implants, idPrefix, label }: { implants: ActiveImplant[]; idPrefix: string; label: string }) {
  ...
  return (
    <svg viewBox="0 0 640 330" role="img" aria-label={`${label} 조건의 buried gate cell transistor implant 농도 분포`}>
      ...
      {implants.map((implant,index) => { const model=SOURCE_MODEL[implant.source]??SOURCE_MODEL.Boron; const cy=52+(model.rp(implant.energy)/.58)*234; const doseRatio=implant.dose/2.6e13; const gradId=`${doseId}-${index}`; const opacity=clamp(0.4+0.35*doseRatio,0.4,1); const sizeScale=clamp(0.75+0.35*doseRatio,0.75,1.5); const rx=184*sizeScale, ry=72*sizeScale; const innerOpacity=clamp(0.72+0.16*doseRatio,0.72,0.98); return <g key={`${implant.source}-${implant.energy}-${index}`}><defs><radialGradient id={gradId}><stop offset="0%" stopColor="#ef3f23" stopOpacity={innerOpacity}/><stop offset="36%" stopColor="#ffc43d" stopOpacity=".9"/><stop offset="62%" stopColor="#29d5a4" stopOpacity=".74"/><stop offset="100%" stopColor="#1f70ff" stopOpacity="0"/></radialGradient></defs><ellipse cx="320" cy={cy} rx={rx} ry={ry} fill={`url(#${gradId})`} opacity={opacity} filter={`url(#${glowId})`}/><line x1="468" x2="566" y1={cy} y2={cy} stroke={index===0?"#e04f2c":"#008fb0"} strokeWidth="2" strokeDasharray="5 5"/><text x="570" y={cy+4} fontSize="12" fill="#28424f" textAnchor="end">{`${implant.energy} keV peak`}</text></g>; })}
